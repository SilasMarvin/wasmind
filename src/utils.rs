...
pub fn filter_wait_tool_calls(messages: &[ChatMessage]) -> Vec<ChatMessage> {
    ...
    for message in messages {
        if let ChatMessage::Tool { tool_call_id, name, .. } = message {
            if name == WAIT_TOOL_NAME && wait_call_ids_to_filter.contains(tool_call_id) {
...
mod tests {
    use super::*;
    use crate::llm_client::{ToolCall, Function};
    ...
    fn test_filter_wait_tool_calls_removes_wait_pairs() {
        let messages = vec![
            ChatMessage::System { content: "System prompt".to_string() },
    ...
    fn test_filter_wait_tool_calls_handles_interleaved_messages() {
        let messages = vec![
            ChatMessage::User { content: "Start".to_string() },
            create_assistant_message(Some(vec![create_tool_call("wait-1", WAIT_TOOL_NAME)])),
            ChatMessage::System { content: "Interleaved".to_string() },
            create_tool_message("wait-1", WAIT_TOOL_NAME),
            ChatMessage::User { content: "End".to_string() },
        ];
    ...
    fn test_filter_wait_tool_calls_handles_orphaned_wait_call() {
        let messages = vec![
            ...
            ChatMessage::User { content: "Next message".to_string() },
        ];
