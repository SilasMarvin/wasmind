...
mod tests {
    ...
    use crate::actors::tools::edit_file::{EditFile, TOOL_NAME as EDIT_FILE_TOOL};
    use crate::actors::tools::file_reader::{
        FileReader, FileReaderActor, TOOL_NAME as READ_FILE_TOOL,
    };
    use crate::actors::tools::complete::Complete;
    use crate::actors::tools::plan_approval::PlanApproval;
    use crate::actors::tools::spawn_agent::SpawnAgent;
    use crate::actors::{
    ...
    async fn test_complete_tool() {
        ...
        let mut seen_awaiting_tools = false;
        let mut seen_complete_received = false;
            ...
            tokio::time::timeout(tokio::time::Duration::from_secs(5), rx.recv()).await
        {
            ...
            match &msg.message {
                    ...
                    ) = &agent_msg.message
                    {
                        match status {
                            ...
                            AgentStatus::Done(result) => {
                                assert!(seen_complete_finished, "Done must come after complete finished");
                ...
                Message::ToolCallUpdate(update) if update.call_id == "complete_call" => {
                    match &update.status {
                        ToolCallStatus::Received { r#type: ToolCallType::TaskCompleted, .. } => {
                            assert!(seen_complete_tool_call, "Complete received must come after tool call");
                            seen_complete_received = true;
                        }
                        ToolCallStatus::Finished(Ok(content)) => {
                            assert!(seen_complete_received, "Complete finished must come after received");
        ...
        assert!(seen_awaiting_tools, "Missing AwaitingTools");
        assert!(seen_complete_received, "Missing complete received");
