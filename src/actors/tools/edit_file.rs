...
pub const TOOL_DESCRIPTION: &str =
    "Applies a list of edits to a file atomically. This is the primary tool for modifying files. Each edit targets a specific line range. The tool processes edits from the bottom of the file to the top to ensure line number integrity during the operation.";
...
pub enum EditFileError {
    ...
    #[snafu(display("Invalid line numbers for edit: start={}, end={}, total_lines={}", start, end, total))]
    InvalidLineNumbers {
        start: usize,
        end: usize,
        total: usize,
...
impl FileEditor {
    ...
    pub fn parse_edits_from_args(args: &serde_json::Value) -> Result<Vec<Edit>> {
        let edits_array = args.get("edits")
        ...
        for edit_obj in edits_array {
            let start_line = edit_obj.get("start_line")
                .and_then(|v| v.as_u64())
                .ok_or_else(|| EditFileError::MissingRequiredField {
                    field: "start_line".to_string(),
                })? as usize;
            
            let end_line = edit_obj.get("end_line")
            ...
            let new_content = edit_obj.get("new_content")
                .and_then(|v| v.as_str())
                .ok_or_else(|| EditFileError::MissingRequiredField {
        ...
        let status = match &result {
            Ok(message) => {
                ...
                if let Ok(canonical_path) = std::fs::canonicalize(path) {
                    let content = file_reader.get_or_read_file_content(&canonical_path, None, None)
...
mod tests {
    ...
    fn test_single_line_replacement() {
        ...
        fs::write(&file_path, "line 1\nline 2\nline 3").unwrap();
        file_reader.read_and_cache_file(&file_path, None, None).unwrap();
