...
fn test_actor_overrides_config_only() {
    ...
    logger_config.insert("format".to_string(), toml::Value::String("json".to_string()));
    ...
    assert_eq!(logger_config.get("level").unwrap().as_str().unwrap(), "debug");
    assert_eq!(logger_config.get("format").unwrap().as_str().unwrap(), "json");
...
fn test_actor_overrides_for_existing_dependency() {
    ...
    override_config.insert("level".to_string(), toml::Value::String("error".to_string()));
    override_config.insert("source".to_string(), toml::Value::String("override".to_string()));
    ...
    let logger_config = logger.config.as_ref().unwrap();
    assert_eq!(logger_config.get("level").unwrap().as_str().unwrap(), "error");
    assert_eq!(logger_config.get("source").unwrap().as_str().unwrap(), "override");
...
fn test_actor_overrides_all_fields() {
    ...
    let mut override_config = toml::Table::new();
    override_config.insert("overridden".to_string(), toml::Value::String("completely".to_string()));
    
    let actor_overrides = vec![
        ActorOverride {
            name: "logger".to_string(),
            source: Some(ActorSource::Path(PathSource {
                path: test_actors_path.join("simple_actor").to_str().unwrap().to_string(),
                package: None,
            })),
            config: Some(override_config),
    ...
    let logger_config = logger.config.as_ref().unwrap();
    assert_eq!(logger_config.get("overridden").unwrap().as_str().unwrap(), "completely");
...
fn test_actor_overrides_partial_fields() {
    ...
    // Source should remain from dependency manifest (via coordinator's Hive.toml)
    match &logger.source {
        ActorSource::Path(path_source) => {
...
fn test_user_defined_actor_separate_from_dependencies() {
    ...
    let actors = vec![
        ...
        Actor {
            ...
            config: {
                ...
                config.insert("custom".to_string(), toml::Value::String("service".to_string()));
                Some(config)
            },
            auto_spawn: true,
            required_spawn_with: vec![],
    ...
    let mut override_config = toml::Table::new();
    override_config.insert("from_override".to_string(), toml::Value::String("override_value".to_string()));
    ...
    let logger_config = logger.config.as_ref().unwrap();
    assert_eq!(logger_config.get("from_override").unwrap().as_str().unwrap(), "override_value");
    assert_eq!(logger.auto_spawn, false);
    assert_eq!(logger.actor_id, "test:logger");
    
    // Custom service should use user definition
    let custom_config = custom_service.config.as_ref().unwrap();
    assert_eq!(custom_config.get("custom").unwrap().as_str().unwrap(), "service");
...
fn test_actor_overrides_nonexistent_actor() {
    ...
    unused_config.insert("unused".to_string(), toml::Value::String("value".to_string()));
...
fn test_actor_and_override_conflict_error() {
    ...
    let actor_overrides = vec![
        ActorOverride {
            ...
            config: Some({
                let mut config = toml::Table::new();
                config.insert("should_not_work".to_string(), toml::Value::String("error".to_string()));
                config
            }),
            auto_spawn: Some(true),
            required_spawn_with: None,
