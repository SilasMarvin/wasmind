use hive_config::{Actor, ActorSource, PathSource, GitRef, Repository};
...
fn test_dependency_resolution_with_path_sources() {
    ...
    assert_eq!(resolved["coordinator_instance"].actor_id, "test:coordinator");
...
"#;
    ...
    // Check actor IDs
    assert_eq!(resolved["package_actor_instance"].actor_id, "test:package-actor");
...
fn test_package_manifest_not_found() {
    ...
    let package_dir = workspace_path.join("crates").join("missing_manifest_package");
    fs::create_dir_all(&package_dir).unwrap();
...
fn test_load_manifest_for_source_directly() {
    ...
    fs::write(workspace_path.join("Hive.toml"), r#"actor_id = "test:root""#).unwrap();
