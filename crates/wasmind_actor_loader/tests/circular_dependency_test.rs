...
async fn test_circular_dependency_detection() {
    ...
    // Create Actor A directory with Wasmind.toml
...
source = { path = "../actor_b" }
"#;
    fs::write(actor_a_dir.join("Wasmind.toml"), actor_a_manifest).unwrap();

    // Create Actor B directory with Wasmind.toml (depends on A via different logical name)
...
source = { path = "../actor_a" }
"#;
    fs::write(actor_b_dir.join("Wasmind.toml"), actor_b_manifest).unwrap();
...
source = { path = "../actor_a" }
"#;
    fs::write(actor_c_dir.join("Wasmind.toml"), actor_c_manifest).unwrap();
