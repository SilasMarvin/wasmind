...
"#;
    ...
    match result {
        ...
        Err(e) => {
            ...
            if error_msg.contains("git") || error_msg.contains("clone") || error_msg.contains("Failed to execute command") {
                panic!("❌ Failed due to git clone attempt - optimization not working: {}", error_msg);
            } else {
                // Other errors (like missing dependency) are acceptable for this test
                println!("ℹ️  Failed due to dependency issues (expected): {}", error_msg);
            }
        }
    }
}
...
async fn test_same_git_source_different_names_use_same_cache() {
    ...
    assert_eq!(hash1, hash2, "Same git source should have same cache key regardless of actor name");
    
    // Different actor names should NOT affect the source hash
    assert_ne!(&actor1.name, &actor2.name, "Sanity check: actor names are different");
