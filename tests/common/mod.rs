...
pub fn create_test_config_with_mock_endpoint(mock_endpoint: String) -> ParsedConfig {
    ...
    config.hive.sub_manager_model.system_prompt = Some("{{id}}".to_string());
    config.hive.worker_model.system_prompt = Some("{{id}}".to_string());
...
impl<'a> MockSequence<'a> {
    ...
    pub fn new(mock_server: &'a MockServer, agent_scope: Scope, user_message: impl Into<String>) -> Self {
        Self {
            mock_server,
            agent_scope,
    ...
    pub fn responds_with_tool_call(mut self, response_id: impl Into<String>, call_id: impl Into<String>, tool_name: impl Into<String>, args: Value) -> Self {
    ...
    pub fn then_expects_tool_result(mut self, call_id: impl Into<String>, result: impl Into<String>) -> Self {
        self.conversation_history.push(ConversationStep::ToolResult {
            call_id: call_id.into(),
            result: result.into(),
        });
        self
    ...
    pub fn responds_with_read_file(self, response_id: impl Into<String>, call_id: impl Into<String>, file_path: &str) -> Self {
        self.responds_with_tool_call(response_id, call_id, "read_file", json!({
            "path": file_path
        }))
    }

    /// Convenience method for edit_file tool call
    pub fn responds_with_edit_file(self, response_id: impl Into<String>, call_id: impl Into<String>, file_path: &str, action: &str, replacement_text: &str) -> Self {
        self.responds_with_tool_call(response_id, call_id, "edit_file", json!({
            "path": file_path,
            "action": action,
    ...
    pub fn responds_with_complete(self, response_id: impl Into<String>, call_id: impl Into<String>, summary: &str, success: bool) -> Self {
        self.responds_with_tool_call(response_id, call_id, "complete", json!({
            "summary": summary,
            "success": success
        }))
    }
    ...
    pub fn responds_with_approve_plan(self, response_id: impl Into<String>, call_id: impl Into<String>, agent_id: &str) -> Self {
        self.responds_with_tool_call(response_id, call_id, "approve_plan", json!({
            "agent_id": agent_id
        }))
    }

    /// Convenience method for spawn_agents tool call
    pub fn responds_with_spawn_agents(self, response_id: impl Into<String>, call_id: impl Into<String>, agents: Vec<Value>, wait: bool) -> Self {
        self.responds_with_tool_call(response_id, call_id, "spawn_agents", json!({
    ...
    pub async fn build(self) {
        ...
        // Find only ToolCall steps and mount mocks for them
        for (i, step) in self.conversation_history.iter().enumerate() {
            if let ConversationStep::ToolCall { response_id, tool_call } = step {
                ...
                let response_body = create_tool_call_response(response_id, vec![tool_call.clone()]);

                // Create a custom matcher that exactly matches the message count and structure
                        ...
                        if let Ok(body_str) = std::str::from_utf8(&request.body) {
                            if let Ok(body_json) = serde_json::from_str::<Value>(body_str) {
                                if let (Some(actual_msgs), Some(expected_msgs)) = 
                                    (body_json.get("messages"), self.expected_messages.get("messages")) {
    ...
    fn build_expected_messages_for_step(&self, agent_id: &str, step_index: usize) -> Value {
        ...
        // Add conversation history up to this step
        for (i, step) in self.conversation_history.iter().enumerate() {
            if i >= step_index {
                break;
            }
...
pub fn create_mock_sequence(mock_server: &MockServer, agent_scope: Scope, user_message: impl Into<String>) -> MockSequence {
...
fn create_tool_call_response(response_id: &str, tool_calls: Vec<MockToolCall>) -> Value {
    let tool_calls_json: Vec<Value> = tool_calls.into_iter().map(|tc| {
        json!({
            "id": tc.call_id,
            "type": "function",
            "function": {
