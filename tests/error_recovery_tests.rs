...
async fn test_command_tool_failure_recovery() {
    ...
    assert!(exit_code == 0 || exit_code == 124, "System should handle command failures gracefully");
    
    // Verify the system is still responsive after the failure
    let (exit_code, stdout, _) = sandbox.exec_command("echo 'System is still responsive'").await.unwrap();
...
async fn test_file_permission_error_recovery() {
    ...
    sandbox.exec_command("echo 'protected content' > /workspace/temp/protected.txt").await.unwrap();
    sandbox.exec_command("chmod 000 /workspace/temp/protected.txt").await.unwrap();
    ...
    assert!(exit_code == 0 || exit_code == 124, "System should handle permission errors gracefully");
    
    // Check if alternative action was taken
    let (exit_code, stdout, _) = sandbox.exec_command("cat /workspace/temp/alternative.txt").await.unwrap();
    if exit_code == 0 {
        assert!(stdout.contains("Alternative content"), "System should create alternative file when primary action fails");
    }
...
async fn test_agent_communication_failure_recovery() {
    ...
    match result {
        Ok((exit_code, stdout, stderr)) => {
            ...
            assert!(exit_code >= 0, "System should handle interruptions without crashing");
        }
        Err(e) => {
    ...
    let (exit_code, stdout, _) = sandbox2.exec_command("echo 'Sandbox still functional'").await.unwrap();
    assert_eq!(exit_code, 0);
...
async fn test_resource_exhaustion_recovery() {
    ...
    assert!(exit_code >= 0, "System should handle resource exhaustion gracefully");
...
async fn test_invalid_input_handling() {
    ...
    for (i, prompt) in invalid_prompts.iter().enumerate() {
        ...
        let (exit_code, _stdout, _) = sandbox.exec_command("echo 'Still responsive'").await.unwrap();
        assert_eq!(exit_code, 0, "System should remain responsive after invalid input {}", i + 1);
    }
...
async fn test_concurrent_operation_conflicts() {
    ...
    sandbox.exec_command("echo 'Initial content' > /workspace/temp/shared.txt").await.unwrap();
    ...
    assert!(exit_code == 0 || exit_code == 124, "System should handle concurrent operations gracefully");
    
    // Verify file integrity
    let (exit_code, content, _) = sandbox.exec_command("cat /workspace/temp/shared.txt").await.unwrap();
    if exit_code == 0 {
        assert!(content.contains("Initial content"), "Original content should be preserved");
...
async fn test_cleanup_after_failures() {
    ...
    let (exit_code, processes, _) = sandbox.exec_command("ps aux | grep -v grep | wc -l").await.unwrap();
    if exit_code == 0 {
        let process_count: i32 = processes.trim().parse().unwrap_or(0);
        assert!(process_count < 50, "Should not have excessive processes running: {}", process_count);
    }
    ...
    if exit_code == 0 {
        ...
        assert!(fd_count < 1000, "Should not have excessive file descriptors open: {}", fd_count);
    }
    
    // Check memory usage
    let (exit_code, memory, _) = sandbox.exec_command("free -m | grep Mem | awk '{print $3}'").await.unwrap();
    if exit_code == 0 {
        let memory_used: i32 = memory.trim().parse().unwrap_or(0);
        assert!(memory_used < 400, "Should not use excessive memory: {}MB", memory_used);
    }
...
async fn test_network_isolation_and_security() {
    ...
    for (i, cmd) in security_tests.iter().enumerate() {
        ...
        assert!(exit_code >= 0, "Security test {} should complete without system crashes", i + 1);
    }
...
pub async fn run_error_recovery_tests() -> Result<(), String> {
    ...
    let tests = vec![
        ("Command failure recovery", "test_command_tool_failure_recovery"),
        ("File permission errors", "test_file_permission_error_recovery"),
        ("Agent communication failures", "test_agent_communication_failure_recovery"),
        ("Resource exhaustion", "test_resource_exhaustion_recovery"),
        ("Invalid input handling", "test_invalid_input_handling"),
        ("Concurrent operation conflicts", "test_concurrent_operation_conflicts"),
